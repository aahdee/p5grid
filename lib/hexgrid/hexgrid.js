...
function Hex(q, r, s) {
  if (Math.round(q + r + s) !== 0) throw "q, r, s doesn't add to 0";
  return { q: q, r: r, s: s };
}
...
function hexLayout(orientation, size) {
...
//returns the center coordinate of a hex
function hexToScreen(layout, hex) {
  //layout, hex
...
//use b matrix
function screenToHex(layout, pixel) {
  //layout, point
  var ori = layout.orientation;
  var size = layout.size;
  var o = Point(0, 0);
...
function hexGetCorners(layout, hex) {
  ...
  var corners = [];
  var center = hexToScreen(layout, hex);
...
function hexDebugGrid(layout, hexes) {
  ...
  for (var i = 0; i < hexes.length; i++) {
    currHex = hexes[i];
    var point = hexToScreen(layout, currHex);
    x = point.x;
    y = point.y;
    fill(0, 255, 0);
