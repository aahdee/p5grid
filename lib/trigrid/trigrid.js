...
function triToString(tri) {
  if (!isTri(tri)) throw `Input ${tri} is not a valid triangle`;
  return `(${tri.q.toString()}, ${tri.r.toString()}, ${tri.s.toString()})`;
}
...
function triDebugGrid(layout, tris) {
  let currTri, x, y;
  textAlign(CENTER);
  textSize(20);
  for (let i = 0; i < tris.length; i++) {
    currTri = tris[i];
    let point = triToScreen(layout, currTri);
    x = point.x;
    y = point.y;
    fill(0, 255, 0);
    if (currTri.q == currTri.r && currTri.r == currTri.s && currTri.s == 0) {
      text("(q,r,s)", x, y);
      continue;
    }
    text(triToString(currTri), x, y);
    fill(150, 0, 150);
